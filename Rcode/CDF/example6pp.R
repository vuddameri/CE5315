...
ppnames <- c('Hazen','Weibull','Gringorten','Cunnane','Beard') # add column names
colnames(ppx) <- ppnames
...
legend('bottomright',legend= ppnames,
...
risks <- c(Hazen.dry,Weibull.dry,Gringorten.dry,Cunnane.dry,Beard.dry)
names(risks) <- ppnames
round(risks,4)
